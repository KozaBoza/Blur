...
def main():
    ...
    background_prepared = ImageOps.fit(background_pil, new_size, method=Image.Resampling.LANCZOS)
    ...
    try:
        while True:
            ...
            if frame_count % 3 == 0:
                ...
                human, background_seg = Segmentation.segmentation(frame_pil, model, device)

                # Use prepared background
                changed_background = ChangeBackground.change_background(human, background_prepared)
                last_result = cv2.cvtColor(np.array(changed_background), cv2.COLOR_RGB2BGR)
